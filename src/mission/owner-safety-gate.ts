...
export function authorizeAgentAction(input: {
  ...
  try {
    ...
    if (amountCents !== null && amountCents !== undefined) {
      ...
      const policy = currentPolicy();
      const dailySpent = (() => { try { return Number(missionDb.get<{ cents: number }>(`SELECT COALESCE(SUM(amount_cents),0) AS cents FROM mission_ledger WHERE direction='debit' AND substr(created_at,1,10)=?`, [nowIso().slice(0,10)])?.cents ?? 0); } catch { return 0; } })();

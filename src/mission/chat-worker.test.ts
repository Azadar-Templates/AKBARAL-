...
it('only opted-in new owner messages enqueue, idempotent replay and agent replies never enqueue again', () => {
...
it('manual agent replies supersede queued jobs without any quota or money reservation', async () => {
  let called = false;
  const job = await runNextAgentChat(async () => { called = true; return response(); });
...
it('dispatches one budgeted model call and records a durable reply without recording a guessed charge', async () => {
  ...
  const job = await runNextAgentChat(async (permit, signal, request) => {
    ...
    assert.equal(await runNextAgentChat(async () => { throw new Error('must not redispatch'); }), null);
    return response();
  });
  ...
  assert.equal(await runNextAgentChat(), null);
  reconcile(f, job!);
});
...
it('unknown provider failures keep both holds and never fabricate or automatically retry a reply', async () => {
  ...
  const job = await runNextAgentChat(async () => { throw new Error('synthetic secret-like provider error must not be stored'); });
...
it('configuration revocation during a call records usage but withholds the reply', async () => {
  const f = fixture(); f.message();
  const job = await runNextAgentChat(async () => { configureAgentChat(f.agentId, { ...f.config, enabled: false }, owner); return response(); });
...
it('late reply persistence failure rolls back the message and keeps a recoverable non-replayed job', async () => {
  ...
  let job;
  try { job = await runNextAgentChat(async () => response()); } finally { missionDb.run = run; }
  assert.equal(job!.status, 'needs_review'); assert.equal(listAgentMessages(f.agentId).messages.length, 1);
  reconcile(f, job!);
});
...
it('fixed Google adapter uses the encrypted credential only in a header, with no tools or redirects', async () => {
  const f = fixture(); f.message();
  const job = await runNextAgentChat((permit, signal, request) => invokeGoogleChat(permit, signal, request, (async (url, init) => {
...
it('missing usage stays uncertain while a known blocked model response records usage without a reply', async () => {
  for (const actual of [false, true]) {
    const f = fixture(); f.message();
    const job = await runNextAgentChat((permit, signal, request) => invokeGoogleChat(permit, signal, request, (async () => new Response(JSON.stringify({ responseId: `synthetic-${randomUUID()}`, ...(actual ? { usageMetadata: { totalTokenCount: 8 } } : {}), candidates: [{ finishReason: 'SAFETY', content: { parts: [{ text: 'Not a deliverable' }] } }] }))) as typeof fetch));
    assert.equal(job!.status, 'needs_review');

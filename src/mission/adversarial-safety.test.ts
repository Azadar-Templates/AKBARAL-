...
// Credential-shaped fixtures are ASSEMBLED at runtime so this source file
// contains no scannable secret marker — the convention the repo-wide secret
// gate enforces (see src/security/scan-secrets.test.ts, whose docstring states
// it, and src/config/env.test.ts, which does the same). The values are still
// genuine `sk-`-shaped strings at runtime, so the redaction and export-refusal
// assertions below exercise the real thing.
const FAKE_EXPORT_SECRET = ['sk', '1234567890abcdef1234567890'].join('-');
const FAKE_AUDIT_SECRET = ['sk', 'test1234567890abcdef'].join('-');
...
describe('OWNER-SAFETY / LIABILITY GATE — adversarial (fail-closed)', ()=>{
  ...
  it('14. cannot leak KYC documents, credentials or private information', ()=>{
    ...
    const d:any = authorizeAgentAction({
      ...
      payload:{ secret: FAKE_EXPORT_SECRET, cnic:'12345-1234567-1', exfiltrate:'send to external' }
    });
    assert.equal(d.allowed, false);
  ...
  it('immutable/auditable trail: task→agent→authorization→connector→action→provider response→verification→settlement is hash-chained and tamper-evident', ()=>{
    ...
    appendMissionAudit({actorType:'agent', actorId:workAgent, action:'test.secret_attempt', detail:{ secret: FAKE_AUDIT_SECRET, password:'hunter2', token:'ghp_123' } as any});
    const lastAudit = missionDb.get('SELECT detail FROM mission_audit ORDER BY seq DESC LIMIT 1') as any;
    assert.ok(!String(lastAudit.detail).includes(FAKE_AUDIT_SECRET), 'the secret must be redacted out of the audit detail');

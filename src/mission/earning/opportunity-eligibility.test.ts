...
before(()=> {
  ...
  try { const o = provisionOwner({email:`elig-${randomUUID()}@test.local`, password:'StrongPass!123', displayName:'Elig'}); ownerId = o.id ?? (o as { owner?: { id?: string } }).owner?.id; } catch { const { missionDb } = require('../database'); const r = missionDb.get('SELECT id FROM mission_owner LIMIT 1'); if (!r) throw new Error('no mission owner row - test cannot run'); ownerId = String(r.id); }
...
describe('opportunity eligibility + activation gating', ()=> {
  ...
  it('listEligible filters an inserted opportunity', ()=> {
    // `paid_research_data` carries the D2-D4 genuine-evidence guard: a lawful
    // purpose reference, a dataset digest, a resolvable evidence URL and both
    // data-safety attestations are mandatory, and synthetic markers are
    // refused. Supply them so this test exercises eligibility filtering rather
    // than the evidence gate — and assert the gate still refuses without them.
    const lawfulEvidence = {
      lawfulPurposeRef: 'LP-ELIG-RESEARCH-0001',
      datasetSha256: 'a'.repeat(64),
      evidenceUrl: 'https://evidence.elig-research.test/datasets/lp-elig-research-0001',
      nonSensitiveDataOnly: true,
      dataRightsReviewed: true,
    };
    assert.throws(
      ()=> EarningEngine.discoverOpportunity({registryKey:'paid_research_data', provider:'Elig Corp', platform:'Direct Client Research', grossCents:50000, expectedFeesCents:5000, expectedCostsCents:500, paymentMethod:'wire', settlementEvidence:'evidence', opportunityExpiry: expiryIso()}),
      /lawful_purpose_required/,
      'paid_research_data without a lawful purpose reference must be refused',
    );
    const opp:any = EarningEngine.discoverOpportunity({registryKey:'paid_research_data', provider:'Elig Corp', platform:'Direct Client Research', grossCents:50000, expectedFeesCents:5000, expectedCostsCents:500, paymentMethod:'wire', settlementEvidence:'evidence', opportunityExpiry: expiryIso(), evidenceJson: lawfulEvidence});

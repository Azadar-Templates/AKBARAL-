...
export const OPPORTUNITY_REGISTRY: OpportunityClass[] = [
  ...
  score({
    key: 'ai_implementation',
    label: 'AI Implementation (LLM integration, RAG, agent workflows)',
    earningMechanism: 'Client pays for AI integration, RAG pipeline, or agent workflow implemented and delivered with disclosed AI assistance.',
    agentWork: 'Agent builds RAG (Tavily/Brave/Firecrawl), integrates OpenAI/Anthropic/Gemini via OpenRouter, validates via executor/verifier, deploys preview on Modal/Vercel/Cloudflare.',
    customerSource: 'Inbound client or marketplace contract where AI implementation is explicitly requested and disclosed.',
    usdPaymentMechanism: 'Marketplace escrow or direct Stripe/PayPal invoice; enterprise via bank wire.',
    payoutMethodAndSettlementEvidence: 'Platform/Stripe payout → payout-verification + receiving rail credit; net USD only after independent verification.',
    autonomousPermitted: true,
    autonomousNote: 'Permitted where contract allows AI assistance; agent performs implementation, owner handles credentials, production deployment and compliance review.',
    humanControlled: 'Owner approves data sources, model credentials via vault, production keys, compliance review, contract signing, payout.',
    countryRestrictions: 'As per freelance; no additional restrictions; data residency per client contract.',
    apiAutomationAvailability: 'Full (OpenAI/Anthropic/Gemini/OpenRouter, Tavily/Brave/Firecrawl/Browserbase, Modal, GitHub, Cloudflare, Neon/Supabase).',
    accountRequirements: 'Owner-provided model credentials via vault, repo/hosting access, no shared secrets.',
    expectedCosts: 'Inference + hosting ~$0.20-5 per task, no marketplace fee if direct.',
    fraudRisks: 'Hallucinated capabilities, data exfiltration, unauthorized prod changes – blocked by scope hash + vault + human review gate.',
    exclusivelyAssignable: true,
    paymentVerifiable: true,
    status: 'verified',
    integrations: ['OpenAI','Anthropic Claude','Google Gemini','OpenRouter','Tavily','Firecrawl','Browserbase','Modal','GitHub','Cloudflare','Vercel','Neon','Supabase'],
    ranking: {genuinePaidWork:5, automationPermission:5, usdVerifiability:5, accessibility:4, scalability:4, setupRequirements:3, operatingCost:4},
    representativePlatforms: [
      {name:'Upwork AI Services', officialUrl:'https://www.upwork.com/hire/ai-engineers/', evidence:'Directory: AI implementation gigs, escrow, payout verification (upwork.com)'},
      {name:'Contra AI Projects', officialUrl:'https://help.contra.com/en/articles/9322763', evidence:'Official: AI projects, escrow, fee 0% for client (help.contra.com)'},
    ]
  }),
  score({
    key: 'automation_services',
    label: 'Automation Services (workflow, n8n/Zapier/Make, RPA, API glue)',
    earningMechanism: 'Client pays for workflow automation, integration glue, or RPA implemented on permitted APIs with explicit permission.',
    agentWork: 'Agent builds automation via GitHub/Docker, integrates HubSpot/Salesforce/Slack/Gmail APIs where owner-authorized, validates idempotency and error handling, provides runbook.',
    customerSource: 'Direct client with explicit system access grant or marketplace automation gig.',
    usdPaymentMechanism: 'Direct invoice or marketplace escrow.',
    payoutMethodAndSettlementEvidence: 'Stripe/PayPal/bank via marketplace; verified before treasury (same chain).',
    autonomousPermitted: true,
    autonomousNote: 'Automation of owner-authorized systems is permitted; bulk unsolicited outreach, spam, or ToS-violating scraping is blocked and human-gated.',
    humanControlled: 'Owner grants scoped API credentials via vault, approves data flows, reviews idempotency, handles production credentials.',
    countryRestrictions: 'Agnostic; outreach must respect CAN-SPAM/GDPR; payout country per processor.',
    apiAutomationAvailability: 'HubSpot/Salesforce/Slack/Gmail/Google Sheets/Microsoft Graph APIs owner-gated; no bulk without consent.',
    accountRequirements: 'Client-provided scoped access via vault, documented permissions, NDA where required.',
    expectedCosts: 'API + compute ~$0.05-2 per run; platform fees 5-20% if via marketplace.',
    fraudRisks: 'Unauthorized access, spam, data exfiltration – blocked by vault + approval + suppression list + secret scan.',
    exclusivelyAssignable: true,
    paymentVerifiable: true,
    status: 'verified',
    integrations: ['GitHub','Docker','HubSpot','Salesforce','Slack','Gmail','Google Sheets','Microsoft Graph','Zapier via API','Modal','Neon'],
    ranking: {genuinePaidWork:5, automationPermission:5, usdVerifiability:4, accessibility:4, scalability:4, setupRequirements:4, operatingCost:4},
    representativePlatforms: [
      {name:'Upwork Automation', officialUrl:'https://www.upwork.com/freelance-jobs/automation/', evidence:'Directory: automation gigs, escrow (upwork.com)'},
    ]
  }),
  score({
    key: 'consulting_advisory',
    label: 'Consulting / Advisory (technical, AI, architecture reviews)',
    earningMechanism: 'Client pays for expert review, architecture audit, or advisory session where AI-assisted research is disclosed and permitted.',
    agentWork: 'Agent performs source-backed research via Tavily/Brave/Firecrawl, builds audit checklists, drafts findings with citations; human expert reviews and delivers.',
    customerSource: 'Direct client with explicit consulting engagement and NDA where required.',
    usdPaymentMechanism: 'Direct invoice via Stripe/PayPal/bank or marketplace (Contra, Upwork).',
    payoutMethodAndSettlementEvidence: 'Bank wire / Stripe → payout-verification + receiving rail credit; net USD only after verification.',
    autonomousPermitted: true,
    autonomousNote: 'Research/draft automation permitted; client-facing advice, recommendations, and delivery require human expert review and disclosure.',
    humanControlled: 'Owner verifies engagement scope, approves research sources, reviews recommendations, handles client communication and invoice.',
    countryRestrictions: 'Agnostic; advisory subject to professional licensing where applicable; payout country per processor.',
    apiAutomationAvailability: 'Search APIs (Tavily/Brave/Serper/Google Custom Search) + LLM APIs; no platform automation needed.',
    accountRequirements: 'None for offline work; delivery via Docs/Notion/GitHub if client authorizes.',
    expectedCosts: 'LLM + search cost ~$0.05-1 per report; no platform fees if direct.',
    fraudRisks: 'Unlicensed advice, hallucinated citations, undisclosed AI – blocked by fact-check + human approval gate.',
    exclusivelyAssignable: true,
    paymentVerifiable: true,
    status: 'candidate',
    integrations: ['Tavily','Brave Search','Firecrawl','OpenAI','Anthropic Claude','Google Gemini','OpenRouter','Google Docs','Excel Build'],
    ranking: {genuinePaidWork:4, automationPermission:4, usdVerifiability:4, accessibility:4, scalability:3, setupRequirements:4, operatingCost:4},
    representativePlatforms: [
      {name:'Contra Consulting', officialUrl:'https://help.contra.com/en/articles/9322763', evidence:'Official: consulting projects, escrow (help.contra.com)'},
    ]
  }),
];

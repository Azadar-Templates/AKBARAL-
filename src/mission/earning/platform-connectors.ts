...
export function discoverForConnector(actor: MoneyActor, input: PursuitInput): PursuitResult {
  ...
  appendMissionAudit({actorType: actor.kind==='owner'?'owner':'agent', actorId: actor.id, action:'connector.discovered', subjectType:'earning_opportunity', subjectId: opp.id, detail:{connectorId: connector!.id, registryKey}});

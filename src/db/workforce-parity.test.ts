...
const workforceFiles = listMigrationFiles().filter((name) => /^(001[89]|002[01])_/.test(name));
...
after(() => {
  ...
  try {
    for (const slug of agentSlugs) {
      for (const table of ['economy_ledger', 'economy_reinvestments', 'economy_workflows', 'economy_comms', 'workforce_image_requests', 'economy_agent_profiles']) {

...
Each viewport covers owner login, native receipt form validation, safely rendered evidence, actual-usage reconciliation, separate financial accounting, unstarted hold cancellation, explicit chat opt-in, durable queued-message display, no document-wide horizontal overflow, no JavaScript page errors, and read-only/anonymous restrictions. No response is faked when the worker is not running.

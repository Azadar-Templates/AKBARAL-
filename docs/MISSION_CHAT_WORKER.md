...
2. Configure a legitimate Google credential in the private encrypted vault, explicitly selecting local `model.call` permission in the owner credential form. This metadata does not grant permissions at Google. Assign and provision an API resource to the intended active mission agent, with explicit `requests` and `tokens` limits and evidenced initial usage.

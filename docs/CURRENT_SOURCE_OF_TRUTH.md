...
> **Latest: 2026-09-20 customer-first implementation.** Continues `99c9458` without adding another connector or changing existing connector code. New private customer workbench, reviewed channel inventory, unpublished service packs, actual bounded local fulfillment and read-only attribution to existing verified USD settlements. Focused **179/179**; full current-source regression pending at this checkpoint. No real customer, publication or earnings claimed. See [customer-first inventory and operating sequence](CUSTOMER_FIRST_EARNINGS.md). Earlier sections remain historical.

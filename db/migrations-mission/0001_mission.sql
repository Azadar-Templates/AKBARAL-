-- ZA141251SA mission schema (0001) — private, separate database.
--
-- Table order follows the foreign-key dependency graph: PostgreSQL requires a
-- referenced table to exist before a referencing table is created, while SQLite
-- resolves such references lazily. Keeping the DDL in dependency order makes one
-- schema definition valid on both engines (see src/mission/database.ts).

...
CREATE TABLE IF NOT EXISTS profiles (
  ...
  trial_started_at TEXT NOT NULL DEFAULT (to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')),
  trial_ends_at TEXT NOT NULL DEFAULT (to_char(now() at time zone 'utc' + interval '30 days', 'YYYY-MM-DD HH24:MI:SS')),
  referral_code TEXT,
...
CREATE OR REPLACE FUNCTION trg_profiles_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
...
FOR EACH ROW EXECUTE FUNCTION trg_profiles_updated_at_fn();
...
CREATE OR REPLACE FUNCTION trg_plans_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
FOR EACH ROW EXECUTE FUNCTION trg_plans_updated_at_fn();
...
CREATE OR REPLACE FUNCTION trg_subscriptions_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
FOR EACH ROW EXECUTE FUNCTION trg_subscriptions_updated_at_fn();
...
CREATE OR REPLACE FUNCTION trg_entitlements_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
FOR EACH ROW EXECUTE FUNCTION trg_entitlements_updated_at_fn();
...
CREATE OR REPLACE FUNCTION trg_model_providers_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
CREATE TRIGGER trg_model_providers_updated_at BEFORE UPDATE ON model_providers
FOR EACH ROW EXECUTE FUNCTION trg_model_providers_updated_at_fn();
...
CREATE OR REPLACE FUNCTION trg_models_updated_at_fn() RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at := to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"');
  RETURN NEW;
END $$ LANGUAGE plpgsql;
...
FOR EACH ROW EXECUTE FUNCTION trg_models_updated_at_fn();
...
CREATE TABLE IF NOT EXISTS workspace_members (
  ...
  invited_at TEXT NOT NULL DEFAULT (to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')),
...
CREATE TABLE IF NOT EXISTS knowledge_items (
  id TEXT PRIMARY KEY,
  rowid BIGSERIAL UNIQUE,  -- explicit surrogate matching SQLite's implicit rowid (FTS mirror key)
  project_id TEXT REFERENCES projects(id) ON DELETE CASCADE,
  user_id TEXT NOT NULL REFERENCES users(id) ON DELETE CASCADE,
  ...
  mime_type TEXT,
  metadata TEXT,
  indexed_at TEXT NOT NULL DEFAULT (to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"')),
  created_at TEXT NOT NULL DEFAULT (to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'))
);
...
-- PostgreSQL port: FTS5 virtual table becomes a real mirror table with a
-- generated tsvector column; search uses websearch_to_tsquery('simple', ?).
CREATE TABLE IF NOT EXISTS knowledge_fts (
  rowid BIGINT NOT NULL PRIMARY KEY,
  content TEXT NOT NULL,
  tsv tsvector GENERATED ALWAYS AS (to_tsvector('simple', content)) STORED
);
CREATE INDEX IF NOT EXISTS idx_knowledge_fts_tsv ON knowledge_fts USING GIN (tsv);

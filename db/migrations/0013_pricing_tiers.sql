...
UPDATE plans SET name = 'Professional', updated_at = strftime('%Y-%m-%dT%H:%M:%fZ','now') WHERE key = 'pro';
...
VALUES
  ('pln_starter', 'starter', 'Starter', 'For individuals putting their first agents to work.', 1000, 'USD', 'month', 25, 10, 1, 1, '{"agentWorld":true,"customCredits":true}', 'active', 10, strftime('%Y-%m-%dT%H:%M:%fZ','now'), strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  ('pln_business', 'business', 'Business', 'For teams running continuous multi-agent operations.', 9000, 'USD', 'month', 250, 150, 15, 10, '{"agentWorld":true,"agentFactory":true,"customCredits":true,"apiAccess":true,"team":true}', 'active', 30, strftime('%Y-%m-%dT%H:%M:%fZ','now'), strftime('%Y-%m-%dT%H:%M:%fZ','now')),
  ('pln_scale', 'scale', 'Scale', 'High-volume execution, automation and API throughput.', 20000, 'USD', 'month', 750, 500, 50, 25, '{"agentWorld":true,"agentFactory":true,"customCredits":true,"apiAccess":true,"team":true,"priority":true}', 'active', 40, strftime('%Y-%m-%dT%H:%M:%fZ','now'), strftime('%Y-%m-%dT%H:%M:%fZ','now'))
